    ...
    def unify(self,trees,context):
        ...
        for i in range(len(trees)):
            if self.hyps[i].type == "e":
                ...
                if not test_tree == trees[i]:
                    print("")
                    print(test_tree, test_tree.value, len(test_tree.leaves))
                    print("")
                    print(trees[i], trees[i].value, len(trees[i].leaves))
                    print("")
                    print(self.hyps[i])
                    print("")
                    ...
                    #not any(self.leaves[i] != target.leaves[i] for i in range(len(self.leaves)))
                    raise Exception ("Trees not equal")
                    #assert test_tree == trees[i], test_tree, trees[i]
    ...
    def read_file_contents(self, file_contents, n=None):
        ...
        while n==None or len(self.propositions) < n:
            ...
            # a provable assertion
                ...
                self.parse_p(current_block, file_contents)        
    ...
    def verify(self,prop, verbose=False):
        ...
        while index<len(proof):
            ...
            # this is a axiom or proposition
                ...
                arity = axiom.arity()
                if arity>0:
                    #Joins the target axiom with previous proof steps
